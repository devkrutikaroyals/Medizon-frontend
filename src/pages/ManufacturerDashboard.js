...
const ManufacturerDashboard = () => {
  ...
  useEffect(() => {
    const fetchManufacturerEmail = async () => {
      try {
        ...
        const response = await axios.get("https://admin-panel-backend-knss.onrender.com/api/auth/me", {
  ...
  const fetchProducts = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`https://admin-panel-backend-knss.onrender.com/api/products/fmanufacturer?t=${Date.now()}`, {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const [productsResponse, { data: allOrders }] = await Promise.all([
        axios.get(`https://admin-panel-backend-knss.onrender.com/api/products/fmanufacturer?t=${Date.now()}`, {
          headers: { Authorization: `Bearer ${getToken()}` },
        }),
        supabase
          .from('product_order')
...
const fetchOrders = async () => {
  ...
  try {
    const productsResponse = await axios.get(
      `https://admin-panel-backend-knss.onrender.com/api/products/fmanufacturer?t=${Date.now()}`,
      {
        headers: { Authorization: `Bearer ${getToken()}` },
      }
    );
  ...
  //       "https://admin-panel-backend-knss.onrender.com/api/products/addProduct",
  //       formData,
  //       {
  //         headers: {
  ...
  const editExistingProduct = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://admin-panel-backend-knss.onrender.com/api/products/${editProduct.id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const deleteProduct = async (id) => {
    ...
    try {
      ...
      await axios.delete(`https://admin-panel-backend-knss.onrender.com/api/products/${id}`, {
        headers: {
      ...
      if (newStatus === 'cancelled' && currentOrder.status !== 'cancelled') {
        ...
        await Promise.all(
          currentOrder.items.map(async (item) => {
            try {
              await axios.put(
                `https://admin-panel-backend-knss.onrender.com/api/products/update-stock/${item.product_id}`,
